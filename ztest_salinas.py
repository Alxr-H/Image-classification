...
def settings(args):
    ...
    args.n_cols = 317  # number of columns
    args.n_rows = 512  # number of rows
    args.n_channels = 204  # number of bands
    ...
    args.id_set = 3  # the index of training sets
...
def main(args):
    ...
    # print(len(testset))  # 9532  # 其中x_data--tensor(9532,200,25,25),y_data--tensor(9532,)
    # print(len(predset))  # 10249
    # print(predset[0])   # 返回两个tensor：样本(四维),标签
    # print(predset[11][0].shape) # torch.Size([200, 15, 15]) torch.Size([])
    ...
    # 保存模型参数
    torch.save(net.state_dict(), "./model/net_parameter_pu.pkl")
    ...
    results_file = 'Results/' + 'pu_2D_pytorch_test' + '_img' + '.mat'
